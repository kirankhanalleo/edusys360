...
<main>
    ...
    <p class="text-muted pt"><b><a href="{{ route('dashboard') }}">Home</a></b> - <a href="{{ route('view.employee.attendance') }}">Employee Attendance</a> </p>
    <div class="data-table large-table" style="margin-bottom:0rem; padding-bottom:2rem; border-bottom:3px solid #2384B4;">
        ...
        <hr>
        <form method="post" action="{{ route('create.employee.attendance') }}"> 
            @csrf
            <div class="row" style="padding-top:1.5rem; width:30%;">
                <div class="form-group">
                    <h3>Attendance Date</h3>
                    <input type="date" name="date"  max={{ today() }} id="date">  
                </div>
            </div>
    </div>
    <div class="data-table large-table">
        <div style="display:flex; justify-content:space-between; ">
            <div><h2>Viewing Attendance Records</h2></div>
            <div class="searchbar">
        ...
        <div class="scroll-table">
                    ...
                    <tr id="div{{ $emp->id }}">
                        ...
                        <td colspan="3">
                            <div style="display: flex; justify-content:space-evenly;">
                                <div> 
                                    <input type="radio" name="status{{ $key }}" value="Present" id="present" checked>
                                    <label for="present{{ $key }}">Present</label>
                                </div>
                                <div>
                                    <input type="radio" name="status{{ $key }}" value="Leave" id="leave">
                                    <label for="leave{{ $key }}">Leave</label>
                                </div>
                                <div>
                                    <input type="radio" name="status{{ $key }}" value="Absent" id="absent">
            ...
            <div class="form-group">
                <button type="submit" class="create" style="margin-top:1.5rem; width:max-content;">Create Attendance</button>
            </div>
        </form>
        </div>
    </div>
