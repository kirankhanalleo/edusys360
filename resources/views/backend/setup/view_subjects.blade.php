...
<main>
    ...
    <p class="text-muted pt"><b><a href="{{ route('dashboard') }}">Home</a></b> - <a href="{{ route('view.subjects') }}">Course</a> </p>
    <div class="data-table large-table">
        <div style="display:flex; justify-content:space-between; ">
            <div><h2>Viewing Subjects</h2></div>
            <div class="searchbar">
                <input type="text" id="search-input" onkeyup="searchBar()" placeholder="Search" class="searchbar">  
                <span class="material-symbols-sharp">search</span> 
            </div>
            <div><button class="add" data-modal-target="#modal-box">Add Subjects</button></div> 
        </div>
        <hr>
                ...
                <thead>
                    ...
                    <tbody>
                        ...
                        @foreach ($allData as $key=>$subject)
                            <tr>
                                <td>{{ $key+1 }}</td>
                                <td width="80%">{{ $subject->name }}</td>
                                <td class="edit-delete">
                                  <a class="view-btn" title="view" href="#"><span class="material-symbols-sharp">visibility</span></a>
                                  <a class="edit-btn" title="edit" href="{{ route('edit.subject',$subject->id) }}"><span class="material-symbols-sharp">edit_square</span></a>
                                  <a class="delete-btn" title="delete" id="deleteSubject" href="{{ route('delete.subject',$subject->id) }}"><span class="material-symbols-sharp">delete</span></a>
                                </td>
                            </tr> 
                        @endforeach
                    </tbody>
    ...
    <div class="modal-box" id="modal-box">
        <div class="modal-header">
            <div class="title"><h1>Create Subject</h1></div>
        ...
        <div class="modal-body">
            <form action="{{ route('create.subject') }}" method="post">
                ...
                <div class="input-form">
                    ...
                    <div>
                        <button type="submit" class="create">Create Subject</button>
                    </div> 
                </div>
            </form>    
        </div>
