...
class MonthlyFeeController extends Controller
{
    public function ViewMonthlyFee()
    {
        ...
        return view('backend.student.monthlyfee.view_monthly_fee', $data);
    }
    public function ViewMonthlyFeeByClass(Request $request)
    {
        ...
        foreach ($studentData as $key => $data) {
            $admission_fee = FeeCategoryAmount::where('fee_category_id', '2')->where('class_id', $data->class_id)->first();
            ...
            $html[$key]['tdsource'] .= '<a style="color:white; background:#df4759; padding:5px 3px; width:70%; border-radius:5px;" title="Generate Bill" target="_blanks" href="' . route("monthly.fee.payslip") . '?class_id=' . $data->class_id . '&student_id=' . $data->student_id . '&month=' . $request->month . '">Create Bill</a>';
            $html[$key]['tdsource'] .= '</td>';
        }
    ...
    public function ViewMonthlyFeePaySlip(Request $request)
    {
        ...
        $pdf = PDF::loadView('backend.student.monthlyfee.fee_payslip_pdf', $student);
