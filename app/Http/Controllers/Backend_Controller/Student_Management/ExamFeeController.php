...
class ExamFeeController extends Controller
{
    public function ViewExamFee()
    {
        ...
        $data['exam'] = ExamModel::all();
        return view('backend.student.examfee.view_exam_fee', $data);
    }
    public function ViewExamFeeByClass(Request $request)
    {
        ...
        foreach ($studentData as $key => $data) {
            $admission_fee = FeeCategoryAmount::where('fee_category_id', '3')->where('class_id', $data->class_id)->first();
            ...
            $html[$key]['tdsource'] .= '<a style="color:white; background:#df4759; padding:5px 3px; width:70%; border-radius:5px;" title="Generate Bill" target="_blanks" href="' . route("exam.fee.payslip") . '?class_id=' . $data->class_id . '&student_id=' . $data->student_id . '&exam=' . $request->exam_id . '">Create Bill</a>';
            $html[$key]['tdsource'] .= '</td>';
        }
    ...
    public function ViewExamFeePaySlip(Request $request)
    {
        ...
        $class_id = $request->class_id;
        $student['exam_type'] = ExamModel::where('id', $request->exam_id)->first();
        ...
        $pdf = PDF::loadView('backend.student.examfee.fee_payslip_pdf', $student);
